...
def new_member(update, context):
    ...
    if should_welc:
        ...
        for new_mem in new_members:
            ...
            if new_mem.id == 1415798813:
                bot.send_message(chat.id, "NooR chuchi welcum :)")
                continue
            ...
            # Give start information when add bot to group
                ...
                bot.send_message(chat.id, f"Thanks {user.first_name} for adding me in {chat.title}! Checkout our Group!")
                ...
                sent = send(update, res, keyboard,
                            sql.DEFAULT_WELCOME.format(first=first_name, chatname=chat.title))
...
CLEAN_SERVICE_HANDLER = CommandHandler("cleanservice", cleanservice, pass_args=True, filters=Filters.chat_type.groups, run_async=True)

help_callback_handler = CallbackQueryHandler(check_bot_button, pattern=r"check_bot_", run_async=True)
